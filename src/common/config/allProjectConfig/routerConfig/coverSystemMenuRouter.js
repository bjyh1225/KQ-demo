...
const modulesFiles = require.context('@/project/', true, /coverMenuRouter\.js$/);
var modules={};
var modulesList=[];
...
if(modulesList.length>0){
  ...
  if(process.env.VUE_APP_SRC=='src/project/portal'){//取非独立的top1
    modulesList.some(item=>{
      if(item.scope!="independentRouter"&&item.coverMenuRouter){
        modules=item.coverMenuRouter;
        return true;
      }
      else{
        return false;
    ...
    modulesList.some(item=>{
      if(item.scope=="independentRouter"&&process.env.VUE_APP_SRC==item.scopeSRC&&item.coverMenuRouter){
        modules=item.coverMenuRouter;
        return true;
      }
      else{
