    ...
    >
      <template slot="title">
        <div class="dialog-title">
          <span>{{$t('webAppBuilder.CustomizeSearchBar')}}</span>
        </div>
      </template>
        ...
        <el-col :span="16" class="dialog-body-col form-area">
          <el-scrollbar class="scroll">
            ...
            >
              ...
              <el-form-item :label="$t('webAppBuilder.FilterField')+'：'">
                <el-select
                  multiple
                  v-model="formInfo.filterWhere"
                  style="width:100%;"
              ...
              <el-form-item :label="$t('webAppBuilder.SearchBarTips')+'：'">
                <el-input
                  :placeholder="$t('webAppBuilder.SeparateMultipleFields')"
                  v-model="formInfo.tooltip"
                ></el-input>
              </el-form-item>
            </el-form>
...
export default {
  ...
  methods: {
    ...
    addSet() {
      ...
      this.$refs.searchBarForm.validate(valid => {
        if (valid) {
          this.saveSet();
        } else {
          return false;
    ...
    saveSet() {
      if (!this.searchBarConfigJSON.config) {
        this.searchBarConfigJSON.config = {};
        this.searchBarConfigJSON.config.layers = {};
        this.searchBarConfigJSON.config.layers.layer = [];
      }
      var fields = [];
      var fields_alias = [];
      ...
      var data = {
        ...
        tooltip: {
          __text: this.formInfo.tooltip
        },
        filter_where_field: {
          __text: this.formInfo.filterWhere.toString()
        },
        server_type: {
          __text: "kq"
        }
      ...
      //处理过滤字段
      var str = "";
      this.formInfo.filterWhere.forEach(item => {
        if (str) {
          str = str + " OR " + item + " LIKE '%[VALUE]%'";
        } else {
          str = item + " LIKE '%[VALUE]%'";
        }
      });
      ...
      if (
        ...
        this.searchBarConfigJSON.config.layers.layer.push(
          JSON.parse(JSON.stringify(data))
        );
        data.index = this.searchBarConfigJSON.config.layers.layer.length - 1;
        this.searchBarConfigMap[
          data._src_name +
            data._currentLayerId +
            "_" +
    ...
    deleteSet(data) {
      ...
      if (
        ...
        var index = this.searchBarConfigMap[
          data._src_name +
            data._id +
            "_" +
            data._parentLayerId +
        ...
        this.searchBarConfigJSON.config.layers.layer.splice(index, 1);
        this.searchBarConfigMap[
          data._src_name +
            data._id +
            "_" +
    ...
    initAllData() {
      //初始化数据
      if (this.searchBarConfigJSON.config) {
        var searchBarData = this.searchBarConfigJSON.config.layers.layer;
        searchBarData.forEach((item, ix) => {
          item.index = ix;
          this.searchBarConfigMap[
            item._src_name +
    ...
    initForm(info) {
      //初始化form
      if (info.tooltip.__text) {
        this.formInfo.tooltip = info.tooltip.__text;
      }
      ...
      if (info.filter_where_field.__text) {
        this.formInfo.filterWhere = info.filter_where_field.__text.split(",");
      }
    ...
    resetForm() {
      if (this.$refs.searchBarForm && this.$refs.searchBarForm.clearValidate) {
        this.$refs.searchBarForm.clearValidate();
      }
      this.fieldTableList = [];
      this.fieldList = [];
