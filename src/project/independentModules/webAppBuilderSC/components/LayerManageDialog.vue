    ...
    >
      <template slot="title">
        <div class="dialog-title">
          <span>{{$t('webAppBuilder.LayerManagement')}}</span>
        </div>
      </template>
            ...
            if (data.resultcode && data.resultcode == "error") {
              ...
              layersList.forEach(item => {
                var tempItem = {
                  ...
                  _parentLayerId: item.parentLayerId,
                  _opacity: 1,
                  _icon: item.shapeType == "polyline" ? "line" : item.shapeType,
                  _type: item.layerType,
                  _visible: "0",
                  _is_dynamic: Number(item.isDynamic),
                  _service_type: this.currentSelectedService.C_SERVICE_TYPE,
                  _service_name: this.currentSelectedService.C_NAME,
                  _is_mini: "0",
                  _service_id: this.currentSelectedService.C_ID
                };
                if (item.compositeLayer === true) {
                  if (item.combined === true && item.parentCombined === false) {
              ...
              // this.currentSelectedServiceLayerAllData=data;
