  ...
  <div>
    ...
    <div class="area">
      <div class="title app-aside-border-bottom">
        <div
          class="title-font app-aside-title-font-color app-aside-title-icon"
        >{{$t('webAppBuilder.Thematicanalysis')}}</div>
      </div>
      <div class="content">
        <el-row class="widget">
          <template v-for="item in widgetList">
            <el-col
              :span="8"
              :key="item._widget_id"
              v-if="item._iswidget=='1'&&item._widge_type=='monographicAnalysis'"
            >
              <div
                class="widget-container"
                :class="{'is-hidden':item._isshow=='0'}"
    ...
    <div class="area">
      <div class="title app-aside-border-bottom">
        <div
          class="title-font app-aside-title-font-color app-aside-title-icon"
        >{{$t('webAppBuilder.Bigdataanalysis')}}</div>
      </div>
      <div class="content">
        <el-row class="widget">
          <template v-for="item in widgetList">
            <el-col
              :span="8"
              :key="item._widget_id"
              v-if="item._iswidget=='1'&&item._widge_type=='bigDataAnalysis'"
            >
              <div
                class="widget-container"
                :class="{'is-hidden':item._isshow=='0'}"
    ...
    ></search-bar-dialog>
    <sc-widget-dialog
      :dialogVisible="scWidgetDialogVisible"
      @dialogClose="editDialogClose"
      :layerTree="layerTree"
      :allLayerConfigJSON="allLayerConfigJSON"
      :dialogConfig="scWidgetConfigInfo"
    ></sc-widget-dialog>
  </div>
</template>
...
import ScWidgetDialog from './widget/ScWidgetDialog';
import scWidgetConfig from "../config/scWidgetConfig.js"
export default {
  name: "Widget",
  components: {
    InfoQueryDialog,
  ...
  data() {
    return {
      ...
      editDialogList: {
        ...
        "tdlyxz_monographicAnalysis":"scWidgetDialogVisible",
        "tdlyztgh_monographicAnalysis":"scWidgetDialogVisible",
        "yjxjbnt_monographicAnalysis":"scWidgetDialogVisible",
        "sthx_monographicAnalysis":"scWidgetDialogVisible",
        "lnpdfw_monographicAnalysis":"scWidgetDialogVisible",
        "wpjwftb_monographicAnalysis":"scWidgetDialogVisible",
        "fykczydc_monographicAnalysis":"scWidgetDialogVisible",
        "ydgy_monographicAnalysis":"scWidgetDialogVisible",
        "wfydclqkfx_monographicAnalysis":"scWidgetDialogVisible",
        "tdlyxz_bigDataAnalysis":"scWidgetDialogVisible",
        "tdlyztgh_bigDataAnalysis":"scWidgetDialogVisible",
        "yjxjbnt_bigDataAnalysis":"scWidgetDialogVisible",
        "sthx_bigDataAnalysis":"scWidgetDialogVisible",
        "lnpdfw_bigDataAnalysis":"scWidgetDialogVisible",
        "wpjwftb_bigDataAnalysis":"scWidgetDialogVisible",
        "fykczydc_bigDataAnalysis":"scWidgetDialogVisible",
        "ydgy_bigDataAnalysis":"scWidgetDialogVisible",
        "wfydclqkfx_bigDataAnalysis":"scWidgetDialogVisible"
      },
      layerTree: [],
      scWidgetConfigInfo:{}//四川项目微件配置
    };
  },
  watch: {
    activeName() {
  ...
  methods: {
    createData() {
      ...
      scWidgetShowList.forEach(item=>{
        defaultControl[item] = true;
      })
      //end
      if (hideControlList) {
    ...
    edit(item) {
      ...
      if(scWidgetConfig[item._type]){
        this.scWidgetConfigInfo=scWidgetConfig[item._type];
      }
    },
    editDialogClose() {
      //弹出窗关闭
