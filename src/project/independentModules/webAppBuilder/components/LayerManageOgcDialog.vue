      ...
      this.$api.webAppBuilderApi
        ...
        .then(res => {
          ...
          if (typeof dataStr === "string") {
            ...
            if ((data.resultcode && data.resultcode == "error")||(data.Exception.Message)) {
              ...
              this.configJSON.config.map.projection.bound = `${initArrmin[0]} ${initArrmin[1]} ${initArrmax[0]} ${initArrmax[1]}`;
                ...
                var index='';
                if(wmsprjs instanceof Array){
                  index=wmsprjs[0].substring(wmsprjs[0].lastIndexOf("EPSG:")+5,wmsprjs[0].length);
                }else{
                  index=wmsprjs.substring(wmsprjs.lastIndexOf("EPSG:")+5,wmsprjs.length);
                }
                serviceDetailsInfo._prj =JSON.parse(JSON.stringify(epsgJson)).EPSG[index].prj;
                //更新配置
                this.configJSON.config.map.projection.prj =JSON.parse(JSON.stringify(epsgJson)).EPSG[index].prj;
                ...
                var bigDataprjs =bigData_Capabiliti.crs;
                 var index='';
                if(bigDataprjs instanceof Array){
                  index=bigDataprjs[0].substring(bigDataprjs[0].lastIndexOf("EPSG:")+5,bigDataprjs[0].length);
                }else{
                  index=bigDataprjs.substring(bigDataprjs.lastIndexOf("EPSG:")+5,bigDataprjs.length);
                }
