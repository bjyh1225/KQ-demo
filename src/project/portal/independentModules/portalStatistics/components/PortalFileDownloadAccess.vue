<template>
  <el-row class="portal-map-access">
      ...
      <el-col class="main">
        <div class="choice">
          ...
          <el-button type="primary" :class="!buttonChoiceAll?'graphic':''" @click="handleCommand">
            {{$t('portalStatistics.Selectattachment')}}
        ...
        <div id="graphic-selection">
          <el-radio
            v-model="pieSwitch"
            label="userOlay"
            @change="toggleAngleFun"
            v-show="pieToggleAngle"
          >{{$t('portalStatistics.Displayuserperspective')}}</el-radio>
          <el-radio
            v-model="pieSwitch"
            label="serviceOlay"
            @change="toggleAngleFun"
            v-show="pieToggleAngle"
          >{{$t('portalStatistics.Displayresourceperspective')}}</el-radio>
          <!-- 柱状 1 -->
          <el-button
            type="primary"
            @click="brokenLineFun"
    ...
    <portal-statistics-map-dialog
      ...
      :totalPageTitle="$t('portalStatistics.Attachmentresourcelist')"
    ></portal-statistics-map-dialog>
  </el-row>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    changePageIndexFun() {
      this.$refs.dialogPortal.fileAll();
    },
    // 数组相加合并
    merge(list, name, num) {
      let result = [];
    ...
    getDataServiceStatistcs() {
      this.serviceStatistcsData.dataFileType = this.dataFileType;
      this.serviceStatistcsData.dataFileId = this.dataFileId;
      this.serviceStatistcsData.dataUserId = this.dataUserId;
      this.$api.portalStatisticsApi
        .getDataFilesStatistcs(this.serviceStatistcsData, {
    ...
    pieChartChartAll() {
      ...
      this.pieChartCharPublic(
        legendData,
        seriesType,
        this.$t('portalStatistics.Totaldownloads'),
        "noUser"
      );
    },
    // 饼状图公共
    ...
    pieChartChart() {
      ...
      if (this.dataServiceName.length > 0 && this.dataUserId == "") {
        ...
        this.pieChartCharPublic(
          ...
          this.$t('portalStatistics.Downloads'),
          "noUser"
        );
      } else if (
      ...
      brokenLineEchart.setOption(
        {
          ...
          title: {
            text: this.$t('portalStatistics.Totaldownloads'),
            textStyle: {
              color: "#fff",
              fontSize: 16,
              fontWeight: "normal"
          ...
          legend: {
            data: [this.$t('portalStatistics.AllFiles')],
            textStyle: {
              color: "#fff"
            },
            bottom: 30
          ...
          series: [
            {
              name: this.$t('portalStatistics.AllFiles'),
              type: "bar",
              label: seriesLabel,
    ...
    checkSelection(val) {
      ...
      if (dataServiceIdArr.length > 0 || val.dataUserId.length > 0) {
        // 有资源没有用户
        this.dataFileType = "1";
        this.dataFileId = dataServiceIdArr.join(",");
