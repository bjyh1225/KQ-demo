    ...
    <el-row>
      ...
      <el-col>
        <portal-statistics-num-title-slot
          @changeFun="changeFun"
          @downloadResourcesNumFun="downloadResourcesListHist"
        ></portal-statistics-num-title-slot>
      </el-col>
      <el-col class="main">
        <div class="echart" ref="histogramEchart"></div>
        <div class="table">
          <div class="title">{{$t('portalStatistics.DetailsOfHottestResources')}}</div>
          <!-- 表格 -->
...
export default {
  ...
  data() {
    return {
      titleName: this.$t("portalStatistics.HottestResources"),
      tableData: [],
      tableCol: [
        {
          prop: "name",
  ...
  methods: {
    // 导出
    downloadResourcesListHist(val) {
      var url = "";
      url =
        window.location.href.replace(this.$route.path, "") +
        `/api/Statistcs/select/downloadResourcesListHist?personnelId=${this.$store.getters.userInfo.id}&num=${val.num}&resourseType=${val.resourseType}`;
      window.open(url);
    ...
    changeFun(val) {
      if (!val.num) {
        ...
        this.getStatistcsResourceHist(val);
      }
    },
    // 前端分页
          ...
          seriesList.forEach(list => {
            ...
            res.data.data.forEach(el => {
              if (el.TYPE == list.name) {
                typeArr.push({ value: el.HITS, name: el.NAME });
                number = number + el.HITS;
              }
            });
            list.value = number;
          });
      ...
      histogramEchart.setOption(
        {
          ...
          title: {
            text: this.$t("portalStatistics.HottestResourcesc"),
            textStyle: {
              color: "#fff",
              fontSize: 16,
              fontWeight: "normal"
          ...
          series: [
            {
              name: this.$t("portalStatistics.Visits"),
              type: "pie",
              selectedMode: "single",
              radius: [0, "30%"],
              label: {
