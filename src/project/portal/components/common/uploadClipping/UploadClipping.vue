  ...
  >
    ...
    <el-row class="dialog-body">
      ...
      >
        ...
        <span slot="tip" class="el-upload__tip">{{fileRule.tips+fileRule.size+'kb!'}}</span>
      </el-upload>
      <div class="cropper">
  ...
  methods: {
    ...
    save() {
      if (this.option.img == "null") {
        this.$message({
          message: this.$t("service.Pleasechoosefiletoupload"),
          type: "warning"
        });
      } else {
        // 得到blob  并转化为file
    ...
    beforeUpload(file) {
      const isLt5M = file.size / 1024 < this.fileRule.size;
      ...
      if (!isLt5M) {
        this.$message({
          message:
            this.$t("service.TheFileSizeCannotExceed") +
            this.fileRule.size +
            "kb!",
          type: "error"
        });
