    ...
    >
      ...
      <el-row class="dialog-body">
        ...
        >
          ...
          <el-form-item :label="$t('management.WhetherItIsAMicroService')+':'">
            <el-radio-group v-model="serverInfo.springCloud">
              <el-radio :label="'1'">{{$t('webAppBuilder.Yes')}}</el-radio>
              <el-radio :label="'0'">{{$t('webAppBuilder.No')}}</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item :label="$t('management.WhetherAsAProxyServer')+':'">
            <el-radio-group v-model="serverInfo.proxyServer">
              <el-radio :label="'1'">{{$t('webAppBuilder.Yes')}}</el-radio>
              <el-radio :label="'0'">{{$t('webAppBuilder.No')}}</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item :label="$t('management.WhetherAsAHostingServer')+':'">
            <el-radio-group v-model="serverInfo.trusteeship">
              <el-radio :label="'1'">{{$t('webAppBuilder.Yes')}}</el-radio>
              <el-radio :label="'0'">{{$t('webAppBuilder.No')}}</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item :label="$t('management.WhetherAsAMonitoringServer')+':'">
            <el-radio-group v-model="serverInfo.monitored">
              <el-radio :label="'1'">{{$t('webAppBuilder.Yes')}}</el-radio>
              <el-radio :label="'0'">{{$t('webAppBuilder.No')}}</el-radio>
            </el-radio-group>
          </el-form-item>
          <el-form-item :label="$t('management.rankId')+':'" prop="sortNo">
            <el-input-number v-model="serverInfo.sortNo" :min="1" :max="10"></el-input-number>
          </el-form-item>
  ...
  data() {
    var checkServerIp = (rule, value, callback) => {
      if (this.operate == "edit") {
        ...
        //   return callback(
        //     new Error(this.$t("myCenter.ThisIPcannotbesetasGISIPnow"))
        //   );
        // }
        if (this.serverInfo.serverPortnumber && this.serverInfo.serverHttp) {
    ...
    // var checkServerPort = (rule, value, callback) => {
    //   if (this.serverInfo.serverIp && this.serverInfo.serverHttp) {
    //     this.$api.manageServerApi
    //       .getTypeAllListOnly({
    //         serverIp: this.serverInfo.serverIp,
    //         serverPortnumber: this.serverInfo.serverPortnumber,
    ...
    return {
      ...
      rules: {
        serverIp: [
          {
            required: true,
            message: this.$t("management.PleaseEnterServerIP"),
            trigger: "blur",
          },
          {
            pattern: /^((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})(\.((2(5[0-5]|[0-4]\d))|[0-1]?\d{1,2})){3}$/,
            message: this.$t(
              "management.TheServerIPEnteredIsIncorrectInFormat"
            ),
            trigger: "blur",
          },
          {
        ...
        serverPortnumber: [
          {
            required: true,
            message: this.$t("management.PleaseEnterServerPort"),
            trigger: "blur",
          },
          {
            pattern: /^([0-9]|[1-9]\d|[1-9]\d{2}|[1-9]\d{3}|[1-5]\d{4}|6[0-4]\d{3}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/,
            message: this.$t(
              "management.TheServerPortEnteredIsIncorrectInFormat"
            ),
            trigger: "blur",
          },
          {
        ...
        name: [
          {
            required: true,
            message: this.$t("management.PleaseEnterServerName"),
            trigger: "blur",
          },
          {
            max: 50,
        ...
        serverHttp: [
          {
            required: true,
            message: this.$t("management.PleaseSelectHTTPProtocol"),
            trigger: "change",
          },
        ],
        comments: [
          {
            max: 50,
            message: this.$t("management.Theinputcharactersmustbelessthan80"),
            trigger: "blur",
          },
        ],
      },
