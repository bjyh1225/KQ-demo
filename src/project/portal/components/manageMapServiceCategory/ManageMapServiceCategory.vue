...
export default {
    ...
    return {
      ...
      rules.execute([
        ...
        },
        {
          ...
          api:this.$api.manageServiceCategoryApi.checkTypeName,
          params:{
      ...
      checkComments: (rule, value, callback) => {
        ...
        value='';
         ...
         let rules=new this.formRules.baseValidateRule({
        errorMsg:this.$t('management.Errorinverifyingthecategorydescription'),
        value:value,
        callback:callback
      });
      rules.execute([
  ...
  methods: {
    ...
    getServiceCategory() {
      this.resetInfo();
      this.$api.manageServiceCategoryApi
        .getServiceCategory({
        ...
        .then(res => {
          ...
          var root = {
            parentId: "-1",
            id: 0,
            typeName: this.$t("management.categoriesOfMapServices"),
            comments: ""
          };
          data.push(root);
    ...
    saveTree(data){
      ...
      this.$api.manageServiceCategoryApi
        .saveServiceCategoryAll(
          { serviceCategoryListAllJson:JSON.stringify(this.categoryTree[0].children) },
          {
            loadingText: '正在保存'
          }
        )
