    ...
    <el-row class="service-btn-group">
      ...
      <el-col :span="10" class="add-service-btn-group">
        <el-button
          type="primary base-btn"
          @click="register"
        >{{$t('myCenter.Registerthedataservice')}}</el-button>
      </el-col>
    </el-row>
    <el-row>
      <el-table :data="serviceList" border @selection-change="selectionChange" ref="multipleTable">
        ...
        <el-table-column prop="cz" :label="$t('myCenter.Operate')" min-width="12%">
          <template slot-scope="scope">
...
import PersonalDataServiceDialog from "components/personalDataService/PersonalDataServiceDialog";
  ...
  watch: {
    ...
    dialogVisible() {
      if (!this.dialogVisible) {
        ...
        this.registerInfo.type = "dataService";
              ...
              if (res.status == 200) {
                ...
                res.data.data.forEach((element) => {
                  element.forEach((item) => {
                    ...
                    item.message =
                      this.$store.getters.userInfo.username +
                      this.$t("myCenter.DeletedTheThreeDServiceResourceOf") +
                      selectedName +
                      this.$t("myCenter.DeletedTheThreeDServiceResourceEnd");
