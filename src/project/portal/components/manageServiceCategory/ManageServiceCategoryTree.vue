          ...
          <span class="custom-tree-node" slot-scope="{ node, data }">
            ...
            <span>
              <el-button
                size="mini"
                type="text"
                class="el-icon-portal-icontuozhuai base-btn-text"
              ...
              <el-button
                ...
                size="mini"
                type="text"
                class="el-icon-portal-baocun base-btn-text"
                :class="{'root-hide':data.parentId=='-1'?false:true}"
                @click="saveTree(data)"
              ></el-button>
              <el-button
                ...
                size="mini"
                type="text"
                class="el-icon-portal-quxiaobaocun base-btn-text"
                :class="{'root-hide':data.parentId=='-1'?false:true}"
              ...
              <el-button
                ...
                :class="{'root-hide':data.parentId=='0'||data.parentId=='-1'?false:true}"
                @click="add(data)"
                v-show='!draggable'
              ></el-button>
              <el-button
...
export default {
  ...
  methods: {
    ...
    querySearch(queryString, cb) {
      ...
      if (results.length == 0) {
        results.push({ value: this.$t("management.noResultsFound") });
      }
      cb(results);
    },
    handleSelect(item) {
    ...
    handleDrop() {
      let arr = [];
      arr=JSON.parse(JSON.stringify(this.categoryTree));
      if(arr[0].children&&arr[0].children.length>0){
        arr[0].children.forEach((item,i)=>{
          item.parentId=0;
          item.sortNo=i;
          if(item.children&&item.children.length>0){
            item.children.forEach((childrenitem,ic)=>{
              childrenitem.parentId=item.id;
              childrenitem.sortNo=ic;
            })
          }
        })
      }
      this.$emit("handleDrop", arr);
    }
  },
  data() {
    return {
