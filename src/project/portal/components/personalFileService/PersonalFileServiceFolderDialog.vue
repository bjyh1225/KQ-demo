      ...
      <el-row class="dialog-body" v-show="!nextShow">
        ...
        >
          ...
          <el-form-item :label="'文件夹名称'+':'" prop="folderName">
            <el-input v-model="registerInfo.folderName"></el-input>
          </el-form-item>
          <el-form-item :label="$t('management.Upload')+':'" prop="upload">
            <el-button
              ...
              class="base-btn"
              @click="formUploadBtn"
              id="filebrowseuploadfolder"
            >{{$t('management.ClickToUpload')}}</el-button>
            <el-row
              class="from-progress"
              v-show="filesId.length>0"
            ...
            >
              ...
              <el-col :span="20" v-show="item.percent==100" class="progress-success">
                <i class="el-icon-portal-duihao1"></i>
                {{$t('management.Uploadedsuccessfully')}}
              </el-col>
              <el-col :span="18" v-show="item.percent!=100">
                <el-progress
                  :text-inside="true"
                  :stroke-width="16"
              ...
              <el-col :span="2" v-show="item.percent!=100">
                <el-tooltip
                  :content="$t('management.delete')"
                  placement="bottom"
                  effect="light"
                  @click.native="closeUploadOnly(item)"
                >
...
export default {
  name: "PersonalFileServiceFolderDialog",
  ...
  methods: {
    ...
    closeUploadOnly(val){
      var flistOnly=[];
      this.fileUploadList.forEach(flist=>{
        if(flist.id!=val.id){
          flistOnly.push(flist);
        }
      })
      this.fileUploadList=flistOnly;
    ...
    save() {
      ...
      this.$emit("save", {
        referenceId: this.fileUploadList,
