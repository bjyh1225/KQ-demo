    ...
    >
      <template slot="title">
        <div class="dialog-title">
          <span>{{$t('management.EditFile')}}</span>
        </div>
      </template>
...
export default {
  name: "PersonalFileServiceEditDialog",
  components: {
    UploadClippingNext
  },
  props: {
      ...
      if (this.arr.length != 0) {
        this.arr.forEach(item => {
          ...
          item.children.forEach(ic => {
            if (ic.id == this.resourceData.fileClassify) {
              this.editInfo.power = ic.id;
              return false;
            }
          });
          }else{
            if (item.id == this.resourceData.fileClassify) {
    ...
    savefileResourceService() {
      this.$api.personalFileServiceApi
        .savefileResourceService({
          ...
          fileClassify: this.resourceData.fileClassify,
          fileScope: this.resourceData.scope,
          fileImg: this.serviceImg,
          referenceId: this.resourceData.referenceId,
          fileComments: this.editInfo.describe,
