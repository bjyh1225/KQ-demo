...
<template>
  <div id="service-center" :style="'margin-top:'+homePageHeaderHeight+'px'">
      ...
      <el-col :span="10">
        <el-row type="flex" justify="space-between" class="map-search">
          <el-col :span="20">
            <div class="inp">
              <el-input
                :placeholder="$t('management.Searchdata')"
                v-model="searchVal"
    ...
    <el-row type="flex" justify="space-around">
      <el-col :span="5">
        <side-bar-menu
          :categoryTitle="$t('management.Datacategory')"
      ...
      <el-col :span="17">
        ...
        <map-resource-display
          ...
          @saveKqServiceTop="saveDataTop"
          @deleteKqServiceTop="deleteDataTop"
          @deleteServiceCollection="deleteFileCollection"
          @order="order"
          @dropdownSortList="dropdownSortList"
          @getListService="getDataList"
      ...
      <service-function-collection ref="collection" @refreshList="searchFun" :resourceType="'data'"></service-function-collection>
    </el-row>
  </div>
</template>
<script>
...
export default {
  data() {
    return {
      ...
      serviceName: this.$t('group.Datas'),
      searchVal: "", //top搜索框值
      serviceClassify: "", //侧边栏
      ...
      pageShow: false,
      all: "全部",
      portalIcon: "el-icon-portal-jichudilishujuku"
    };
  },
  name: "ServiceDatacenterCen",
  components: {
    MapResourceDisplay,
    SideBarMenu,
    Pagination,
  ...
  methods: {
    ...
    getDataKwList() {
      this.$api.serviceDatacenterApi
        .getDataKwList({}, { loadingText: this.$t("service.searching") })
        .then(res => {
         this.myKeyWordData = [];
        ...
        .getDataList(
          {
            ...
            dataClassify: this.serviceClassify?this.serviceClassify:this.serviceClassifyParent,
            dataClassifyParent: this.serviceClassifyParent,
