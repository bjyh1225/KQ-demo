...
<template>
  <div class="service-center" :style="'margin-top:'+homePageHeaderHeight+'px'">
      ...
      <el-col :span="10">
        <el-row type="flex" justify="space-between" class="map-search">
          <el-col :span="20">
            <div class="inp">
              <el-input
                :placeholder="$t('myCenter.Searchmappingapps')"
                v-model="searchVal"
    ...
    <el-row type="flex" justify="space-around">
      <el-col :span="5">
        <side-bar-menu
          :portalIcon="'el-icon-portal-map1'"
          :categoryTitle="$t('myCenter.Mappingcategory')"
          :publicField="$t('myCenter.Publicmapping')"
          :myCenterField="$t('myCenter.Mymapping')"
...
export default {
  data() {
    return {
      MapResourceDisplayProps: {
        ...
        appJumpTitle: this.$t("myCenter.Createmap"),
        appJumpIcon: "el-icon-portal-map1",
        pnav:'',//父导航
        snav:''//子导航
      },
      pageShowProps: {
      ...
      serviceName: this.$t("management.Mapping"),
      searchVal: "", //top搜索框值
      serviceClassify: "", //侧边栏
  ...
  methods: {
    ...
    clickServiceDetails(val) {
      this.$emit("getMapbuilderDetails", val);
    },
