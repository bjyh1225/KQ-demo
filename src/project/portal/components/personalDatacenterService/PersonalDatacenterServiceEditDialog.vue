    ...
    >
      <template slot="title">
        <div class="dialog-title">
          <span>{{$t('management.Editdata')}}</span>
        </div>
      </template>
...
export default {
  name: "PersonalDatacenterServiceEditDialog",
  components: {
    UploadClippingNext,
  },
  props: {
  ...
  data() {
    var checkFileEditName = (rule, value, callback) => {
    ...
    var checkFileEditDescribe = (rule, value, callback) => {
      if (value == null) {
        value = "";
      }
      let rules = new this.formRules.baseValidateRule({
    ...
    return {
        ...
        name: [
          {
            required: true,
            trigger: "blur",
            validator: checkFileEditName,
          },
        ],
        describe: [
          {
            trigger: "blur",
            validator: checkFileEditDescribe,
          },
        ],
      },
      fileRule: this.$t("service.OnlyJPGPNGDataUploadedSizeLess1MB") + "1MB",
      autoCropWidth: 236,
      autoCropHeight: 165,
      ...
      serviceImg: "",
      unifiedMark: "datacenter", //缩略图推荐缩略图标识
      inputValue: "",
      dynamicTags: [],
        ...
        this.arr.forEach((item) => {
          if (item.children) {
            item.children.forEach((ic) => {
              if (ic.id == this.resourceData.dataClassify) {
                this.editInfo.power = ic.id;
                return false;
              }
            });
          } else {
            if (item.id == this.resourceData.dataClassify) {
    ...
    saveData() {
      this.$api.personalDatacenterServiceApi
        .saveData({
          ...
          dataClassify: this.resourceData.dataClassify,
          dataScope: this.resourceData.scope,
          dataImg: this.serviceImg,
          referenceId: this.resourceData.referenceId,
          dataComments: this.editInfo.describe,
