      ...
      <el-row class="dialog-body" v-if="!thumbnailShow">
        <el-form :model="editInfo" :rules="rules" ref="editForm" :label-width="formLabelWidth">
          ...
          <el-form-item :label="$t('myCenter.TheCategory')" prop="power">
            <el-select v-model="editInfo.power" :placeholder="$t('myCenter.PleaseSelect')" ref='selectGroup'>
                <el-option
                  v-for="item in arr"
                  :key="item.id"
                  :label="item.typeName"
...
export default {
  name: "ManageBuilderCategoryDetailsEditDialog",
  components: {
    UploadClippingNext
  },
  props: {
  ...
  data() {
    ...
    return {
      ...
      serviceImg: "",
      unifiedMark:'mapDrafting', //缩略图推荐缩略图标识
      inputValue: '',
      dynamicTags:[],
  ...
  watch: {
    ...
    'resourceData.resourceType'(){
      if(this.resourceData.resourceType=='appbuilder'){
        this.unifiedMark='app';
      }else{
        this.unifiedMark='mapDrafting';
      }
    },
    arr() {
  ...
  methods: {
    ...
    savesceneService() {
      if(this.resourceData.resourceType=='scenebuilder'){
       this.$api.webSceneBuilderApi
                .saveSceneAppService({
                  id: this.resourceData.id,
                  name: this.editInfo.name,
                  sceneAppClassify: this.editInfo.power,
                  sceneAppScope: this.resourceData.scope,
                  sceneAppComments: this.editInfo.describe,
                  sceneAppImg: this.serviceImg,
      ...
      else if(this.resourceData.resourceType=='mapbuilder'){
        this.$api.webMapBuilderApi
                  .updataMapbuilder({
                    id: this.resourceData.id,
                    name: this.editInfo.name,
                    mapbuilerClassify: this.editInfo.power,
                    scope: this.resourceData.scope,
                    comments: this.editInfo.describe,
                    img: this.serviceImg,
                    keyWord: this.editInfo.keyWord,
                    personnelId:this.resourceData.personnelId
                  })
    ...
    else if(this.resourceData.resourceType=='bigbuilder'){
      }else if(this.resourceData.resourceType=='appbuilder'){
        this.$api.personalCenterAppApi
        .saveAppService({
          id: this.resourceData.id,
          name: this.editInfo.name,
          appClassify: this.editInfo.power,
          appComments: this.editInfo.describe,
          scope: this.resourceData.scope,
