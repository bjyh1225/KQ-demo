...
//****** file: GeoJSON.js ******
  ...
  var GeoJSON = (function () {
    ...
    function alignProperties (prop) {
      ...
      item.height = prop.buildingHeight || (prop.levels ? prop.levels * METERS_PER_LEVEL : DEFAULT_HEIGHT);
      ...
      read: function (geojson) {
        ...
        for (i = 0, il = collection.length; i < il; i++) {
          ...
          for (j = 0, jl = geometries.length; j < jl; j++) {
            ...
            if (feature._building_id || feature.properties.id) {
              item.id = feature._building_id || feature.properties.id;
            }
  ...
  var Data = {
    ...
    getPixelFootprint: function (buffer) {
      ...
      for (var i = 0, il = buffer.length - 1; i < il; i += 2) {
        // add by liwei
        // 保持和map一样的坐标转换方式
        px = this._map.project([ buffer[ i ], buffer[ i + 1 ] ], this._map.getZoom());
    ...
    set: function (data) {
      ...
      this.addRenderItems(this._staticData[0], true);
    },
    ...
    update: function () {
      ...
      if (this.isStatic && this._staticData) {
        for (var i = 0; i < this._staticData.length; i++) {
          this.addRenderItems(this._staticData[i]);
        }
        return;
      }
...
//****** file: public.js ******
  ...
  // add by liwei
  osmb.initMinZoom = function (config) {
    if (!config)
      return;

    MIN_ZOOM = config._getBuildingMinScaleID();
  }

  // add by liwei
  osmb.prototype.addData = function (geojson) {
    if (!Data._staticData) {
      this.set(geojson);
    }
    else {
      Data._staticData.push(geojson);
      Data.addRenderItems(geojson);
    }
  }

  // add by liwei
  osmb.prototype.getData = function (id) {
    if (Data._staticData) {
      for (var i = 0; i < Data._staticData.length; i++) {
        for (var n = 0; n < Data._staticData[i].features.length; n++) {
          if (Data._staticData[i].features[n]._building_id === id) {
            return Data._staticData[i].features[n];
          }
        }
      }
    }
  ...
  // add by liwei
  osmb.prototype.clearData = function () {
    Data._staticData = [];
    Data.update();
  ...
  // add by liwei
  osmb.prototype.delData = function (id) {
    if (Data._staticData) {
      var foundIndex = -1;
      for (var i = 0; i < Data._staticData.length; i++) {
        for (var n = 0; n < Data._staticData[i].features.length; n++) {
          if (Data._staticData[i].features[n]._building_id === id) {
            Data._staticData[i].features.splice(n, 1);
            delete Data.loadedItems[id];
  ...
  // add by liwei
  osmb.prototype.includes = function (id) {
    if (Data.loadedItems[id])
      return true;
    else
      return false;
  }
